...
Deno.test("score formula uses take-home income remaining after the basket", () => {
  const result = compareStamfordScenario({ ...baseInput, monthlyTakeHomeIncome: 5000 }, stamfordProfiles);
...
Deno.test("fit thresholds label 15 as Tight and 30 as Comfortable", () => {
  const tight = compareStamfordScenario({ ...baseInput, monthlyTakeHomeIncome: 5255.8 }, stamfordProfiles);
  const comfortable = compareStamfordScenario({ ...baseInput, monthlyTakeHomeIncome: 6382.04 }, stamfordProfiles);
...
Deno.test("target rent override changes only the target housing calculation", () => {
  ...
  assertEquals(result.alternatives.find((alternative) => alternative.zip === "06906")?.breakdown.rent, 2350);
});

Deno.test("negative remaining dollars stay signed while the score is clamped to zero", () => {
  const result = compareStamfordScenario({ ...baseInput, monthlyTakeHomeIncome: 3000 }, stamfordProfiles);
...
Deno.test("alternatives are ranked by remaining dollars and exclude the target ZIP", () => {
  ...
  assertEquals(result.alternatives.map((alternative) => alternative.zip), ["06906", "06907", "06905", "06903", "06901"]);
  assertEquals(result.alternatives.some((alternative) => alternative.zip === result.target.zip), false);
});
...
Deno.test("ties are sorted by ZIP for deterministic recommendation order", () => {
  ...
  assertEquals(result.alternatives.slice(0, 2).map((alternative) => alternative.zip), ["06905", "06907"]);
...
Deno.test("no-improvement state is returned when the current ZIP is already best", () => {
  const result = compareStamfordScenario({ ...baseInput, targetRent: 2000, targetZip: "06906", workZip: "06906", commuteDaysPerWeek: 0 }, stamfordProfiles);

  assertEquals(result.bestAlternative, undefined);
  assertEquals(result.noImprovementMessage, "AVA found no better fit within the six-ZIP Stamford demo area.");
});

Deno.test("validation rejects invalid inputs", () => {
  assertThrows(() => compareStamfordScenario({ ...baseInput, monthlyTakeHomeIncome: 0 }, stamfordProfiles));
  assertThrows(() => compareStamfordScenario({ ...baseInput, groceries: -1 }, stamfordProfiles));
  assertThrows(() => compareStamfordScenario({ ...baseInput, targetZip: "10001" }, stamfordProfiles));
  assertThrows(() => compareStamfordScenario({ ...baseInput, householdSize: 1.5 }, stamfordProfiles));
  assertThrows(() => compareStamfordScenario({ ...baseInput, commuteDaysPerWeek: 8 }, stamfordProfiles));
});
